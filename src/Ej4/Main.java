...
public class Main {
	...
	private void exercise1() {
		...
		this.data.addValue(90.0, 0, 2); // Porcentaje de uso del Proceso 1
		
		// 2da fila
		this.data.addValue(14.21, 1, 0); // Inicio del Proceso 2
		this.data.addValue(2.0, 1, 1); // Duración del Proceso 2
		this.data.addValue(80.0, 1, 2); // Porcentaje de uso del Proceso 2
	...
	private void loadAndSortTheDataMatrix() {
		...
		System.out.print("Ingrese el valor de porcentaje de uso para E/S del proceso 1 (valor entero entre 1 y 100): ");
		this.data.addValue(this.scanner.nextDouble(), 0, 2);
		System.out.println();
		
		// Ordenamos los procesos por valor de inicio mientras se van cargando (de menor a mayor).
		Double startOfTheNewProcess, durationOfTheNewProcess, percentageOfUseOfTheNewProcess,
			startOfTheOldProcess, durationOfTheOldProcess, percentageOfUseOfTheOldProcess;
		...
		for (int i=1; i<this.data.getSize(); i++) {
			...
			System.out.print("Ingrese el valor de porcentaje de uso para E/S del proceso " + (i+1) + " (valor entero entre 1 y 100): ");
			percentageOfUseOfTheNewProcess = this.scanner.nextDouble();
			System.out.println();
	...
	private void createUtilizationMatrix() {
		...
		for (int i=0; i<this.quantityOfProcesses; i++) {
			this.utilization.addValue(this.data.getValue(i, 2)/100, i, 0);
			
			Double multiplications = 1.0;
			for (int j=0; j<=i; j++) {
				multiplications *= this.utilization.getValue(j, 0);
			}
			
			this.utilization.addValue(1-multiplications, i, 1);
			this.utilization.addValue(this.utilization.getValue(i, 1) / (i+1), i, 2);
		}
	}
	
	private void showUtilizationMatrix() {
		System.out.println("Tabla de utilización de cada proceso con su % de espera de E/S:\n");
	...
	private void createEventsMatrix() {
		...
		while (! finishedCreatingTheEventsMatrix()) {
			...
			for (int position=0; position<list.size(); position++) {
				Double newValue = this.events.getValue(list.get(position)+1, minute-1) - (this.utilization.getValue(list.get(position), 2)*1);
